defmodule GuardianJwks.HttpFetcher do
  @moduledoc """
  ...
  def fetch_signers(url, opts) do
    ...
    with {:ok, resp} <- Tesla.get(new(opts), url),
      ...
      GuardianJwks.log(:debug, log_level, "JWKS fetching: fetched keys -> #{inspect(keys)}")
      {:ok, keys}
    else
      {:status, status} when is_integer(status) and status >= 400 and status < 500 ->
        GuardianJwks.log(:debug, log_level, "JWKS fetching: #{status} -> client error")
        {:error, :jwks_client_http_error}

      {:status, status} when is_integer(status) and status >= 500 ->
        GuardianJwks.log(:debug, log_level, "JWKS fetching: #{status} -> server error")
      ...
      {:error, :econnrefused} ->
        GuardianJwks.log(:debug, log_level, "JWKS fetching: could not connect (:econnrefused)")
      ...
      error ->
        GuardianJwks.log(:debug, log_level, "JWKS fetching: unkown error #{inspect(error)}")
        error
    end
  end
